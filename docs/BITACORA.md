...
## Sesión 2026-08-19 (11) · Refactor home + Carta del Director + admin sarmat + legal
- **Decisiones del cliente**: Moodle sale de TODA la página (se conservan solo los PDFs rescatados en `content/moodle/files` y `web/public`); hero más "APRECAP" ("Tu futuro en Seguridad Privada"); home con los 4 cursos en 2 secciones (Presenciales+Online: OS-10 y Bastón; Online: Supervisor y CCTV) con cards a las páginas marketing `/cursos/{slug}`; ocultar los cursos no trabajados (Nochero, Electricidad) en todo el sitio público; página "Carta del Director" en el navbar con la historia de Ercio Saavedra; Misión/Visión/Valores con enfoque en DDHH (contra la discriminación) como info para visitantes; intentos ILIMITADOS en todos los cursos (a diferencia de sarmat); NO se agregan códigos ni claims de sarmat (solo "autorizados por OS-10/SENCE/SPD"); panel admin igual al de sarmat (sidebar izquierda) omitiendo Tablón de Chat y lo de "profesor de bastón", conservando los Excel y el editor de diplomas; cumplir Ley 21.719/21.663 con CookieBanner + privacidad/términos + firestore.rules; MD pre-producción con flujo/candados/seguridad estilo sarmat.
- **Moodle eliminado de todo el código**: `data/moodle.ts`, `lib/moodle.ts`, tipos `CursoMoodle`/`ActividadMoodle`, páginas `/campus` y `/cursos-asincronicos` (+ `[slug]`), sección del home, link "Mi Campus" (navbar+footer vía NAV_LINKS) y todas las referencias en `panel/alumno`, `panel/profesor` y `panel/admin` (matriculación/selectores/fallbacks). PDFs rescatados intactos.
- **Vista previa de felicitaciones**: nuevo componente compartido `ResultadoExamen` (tarjeta de resultado de examen, extraída de FinalExam/FinalExamVF que ahora lo usan; se corrigió el fondo oscuro de la tarjeta para que el texto sea legible). Nueva página `/prueba-felicitaciones` con 4 botones (uno por curso) que muestran la pantalla "¡Felicitaciones!" del examen final aprobado con datos de ejemplo. Botón discreto "🧪 Vista previa exámenes" en el navbar (quitar antes de producción, anotado en MD).
- **Home y /cursos**: hero nuevo ("Tu futuro en Seguridad Privada" + respaldo SENCE/SPD/OS-10); 2 secciones de cursos con cards (data nuevo `data/cursos-home.ts`); `/cursos` reescrito con las mismas 2 secciones (sin Nochero/Electricidad). `site.ts` NAV_LINKS: "Carta del Director" agregada, "Mi Campus" eliminada.
- **Carta del Director** (`/carta-del-director`): historia completa de Ercio Saavedra Aravena (1989 → Asesor/Relator → APRECAP 2022), pilares (Experiencia en terreno + Conocimiento + Compromiso), cierre firmado y sección "Nuestra Esencia" con Misión/Visión/Valores renovados con enfoque en derechos humanos, dignidad e inclusión. Bloque Misión/Visión/Valores de `pages.ts` (inicio) actualizado con el mismo enfoque DDHH.
- **Blog**: causa del "mal escrito" = el excerpt del listado incluía la sintaxis markdown cruda de las imágenes (`![...](url)`). Fix: `extractPreview()` en `/blog` que limpia imágenes, links, negritas y encabezados antes de cortar.
- **Panel admin estilo sarmat**: reescritura de `panel/admin/page.tsx` con sidebar izquierda fija (280px, azul marino, grupos "Solicitudes/Usuarios/Clases/Cursos/Reportes/Gestión", ítems con emoji + badge dorado de conteo, footer con usuario y cerrar sesión). Pestañas: Pendientes, Historial (nuevo), Alumnos y Matrículas, Profesores (nuevo filtro por rol), Clases En Vivo, Reuniones Zoom, Diplomas y Certificados (nuevo: generador admin con selector de alumno+curso reutilizando el diploma), Notas y Evaluaciones (nuevo: tabla + export Excel/CSV), Pagos, Contacto, Auditoría. Matriculación ahora con los 4 cursos de la plataforma. Diploma extraído a `components/DiplomaCertificado.tsx` (lo usan alumno y admin).
- **Legal (Ley 21.719 + 21.663)**: nuevo `CookieBanner` global en `layout.tsx` (necesarias/analítica, consentimiento versionado en localStorage); `/privacidad` actualizada (21.663 + medidas: MFA, cifrado TLS, auditoría, control por rol) y `/terminos` con secciones 6 (21.719) y 7 (21.663); nuevo `firestore.rules` en la raíz (reglas por rol para usuarios/solicitudes/enrollments/clases/resultados/pagos/contact/consents y `audit_logs` inmutable) — deploy documentado en el MD pre-producción.
- **MD pre-producción**: `docs/PRE_PRODUCCION_FLUJO_CURSOS.md` con el checklist de candados (módulo→quiz→siguiente módulo, examen final bloqueado, VideoTracker, progreso Firestore, acceso aprobado por admin), intentos ilimitados, umbrales, legal, requisitos Ley 21.659 Art. 46 (pendiente), fotos de cursos y verificación final.
- **Verificación**: `tsc --noEmit` 0 errores; headless en 3000 → **38 checks OK** (home 2 secciones/4 cards sin Moodle ni Test, /cursos sin Nochero/Electricidad, /carta-del-director, /prueba-felicitaciones, /campus y /cursos-asincronicos 404, privacidad/terminos con 21.663, blog sin markdown crudo, regresión evaluaciones/materiales/admin).
- **Pendiente**: commit+push de esta sesión.
...
## Sesión 2026-08-19 (12) · Dominio Cloudflare, Zoho Mail, Administradores Saavedra, Modo Demo y Favicon Oficial

### 1. Rescate de Configuración de Dominio y DNS (Cloudflare + Zoho Mail)
- Auditoría profunda de DNS, host DirectAdmin y correos de la cuenta:
  - **Servidor Hosting antiguo**: IP `66.235.175.220` (`da004.servidoresph.com:2222`).
  - **Zoho Mail corporativo**: registros MX (`mx.zoho.com` pr:10, `mx2.zoho.com` pr:20, `mx3.zoho.com` pr:50), TXT SPF `v=spf1 include:zoho.com ~all` y verificación `zb46898578.zmverify.zoho.com`.
- Guía completa paso a paso redactada en `docs/GUIA_DESPLIEGUE_DOMINIO_CLOUDFLARE.md` (guardada en local y excluida en `.gitignore` para proteger las credenciales).
- Arquitectura confirmada: **Cloudflare Workers** (`@opennextjs/cloudflare` + `wrangler`, worker: `aprecap`).

### 2. Administradores Saavedra incorporados
- Se agregaron a `ADMIN_EMAILS` en `lib/roles.ts`:
  - `conysaavedra.o@gmail.com` (Coni Saavedra)
  - `erciosaavedra@gmail.com` (Ercio Saavedra)
  - `web.aprecap@gmail.com` (Admin central)
- `AuthContext.tsx`: sincronización automática de rol administrativo y elevación de permisos en Firestore.
- Reglas `firestore.rules` y `web/firestore.rules` actualizadas con los 3 correos administradores.

### 3. Eliminación de Modo Demo y Ajuste de Certificación Presencial
- **Modo Demo**: `useModoDemo.ts` desactivado permanentemente para producción; eliminados los cuadros de demo y bypasses en `MiniQuiz.tsx`, `ResultadoExamen.tsx`, `FinalExam.tsx` y `FinalExamVF.tsx`.
- **Certificación diferenciada**:
  - Cursos presenciales (Guardia OS-10 y Bastón y Esposas): se especifica que el Certificado Oficial y la Credencial se entregan presencialmente en APRECAP tras completar las horas prácticas y exámenes ante Carabineros.
  - Cursos online (CCTV y Supervisor): mantienen el flujo de emisión y retiro de diploma tras aprobar la evaluación virtual.

### 4. Favicon e Iconos Oficiales APRECAP
- Generado `favicon.ico` con el escudo oficial de APRECAP mediante script `scripts/generate-favicon.ps1`.
- Sobrescrito `web/src/app/favicon.ico` (se eliminó el icono de Vercel heredado del template) y agregado `web/public/favicon.ico`.
- Generados `web/src/app/icon.png` y `web/src/app/apple-icon.png` en alta resolución.
- Metadatos en `web/src/app/layout.tsx` vinculando tamaños `any`, `32x32`, `192x192`, `512x512` y `apple-touch-icon`.
